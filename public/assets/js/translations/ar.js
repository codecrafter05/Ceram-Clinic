const translations = {
    // Header Navigation
    'Home': 'الرئيسية',
    'home': 'الرئيسية',
    'About Us': 'من نحن',
    'Services': 'الخدمات',
    'services': 'الخدمات',
    'Pages': 'الصفحات',
    'Gallery': 'المعرض',
    'FAQ\'s': 'الأسئلة الشائعة',
    'Contact Us': 'اتصل بنا',
    
    // Hero Section
    'Welcome to': 'مرحباً بكم في',
    'for Excellent Dental Care': 'للعناية الممتازة بالأسنان',
    'More About Us': 'المزيد عنا',
    
    // About Section
    'about us': 'من نحن',
    'read more about us': 'اقرأ المزيد عنا',
    
    // Services Section
    'Our': 'خدماتنا',
    'Our Team': 'فريق',
    'our services': 'خدماتنا',
    'Hight Quality': 'جودة عالية',
    'Services for You.': 'الخدمات لك.',
    'We are committed to sustainability. eco-friendly initiatives.': 'نحن ملتزمون بالاستدامة والمبادرات الصديقة للبيئة.',
    'No services available': 'لا توجد خدمات متاحة',
    'We believe in using the latest technology and techniques to ensure the best outcomes for our patients.': 'نؤمن باستخدام أحدث التقنيات والطرق لضمان أفضل النتائج لمرضانا.',
    'View all services': 'عرض جميع الخدمات',
    
    // Video Section
    'visit clinic': 'زيارة العيادة',
    'Comprehensive Dental Care For All Ages': 'رعاية شاملة للأسنان لجميع الأعمار',
    'play video': 'تشغيل الفيديو',
    
    // Why Choose Us Section
    'why choose us': 'لماذا تختارنا',
    'Diagnosis of': 'تشخيص',
    'Dental Diseases': 'أمراض الأسنان',
    
    // How It Works Section
    'how it work': 'كيف يعمل',
    'No FAQs yet': 'لا توجد أسئلة شائعة بعد',
    'Add FAQs from the admin panel.': 'أضف الأسئلة الشائعة من لوحة التحكم.',
    
    // Team Section
    'Team': 'الفريق',
    'our team': 'فريقنا',
    'Our Friendly': 'فريقنا الودود',
    'Dentists Team': 'من أطباء الأسنان',
    'No team members yet.': 'لا يوجد أعضاء فريق حالياً.',
    'View full team': 'عرض الفريق كاملاً',
    
    // Call To Action Section
    'need dental services ?': 'تحتاج خدمات طب الأسنان؟',
    'opening hours': 'ساعات العمل',
    'More details': 'المزيد من التفاصيل',
    
    // About Us Section
    'about us': 'من نحن',
    'read more about us': 'اقرأ المزيد عنا',
    
    // Video Section
    'play video': 'تشغيل الفيديو',
    
    // Footer
    'quick links': 'روابط سريعة',
    'social media': 'وسائل التواصل الاجتماعي',
    'Useful Links': 'روابط مفيدة',
    'Quick Links': 'روابط سريعة',
    'Services': 'الخدمات',
    'About Us': 'من نحن',
    'Contact': 'اتصل بنا',
    'Privacy Policy': 'سياسة الخصوصية',
    'Terms of Service': 'شروط الخدمة',
    'All Rights Reserved': 'جميع الحقوق محفوظة',
    'The goal our clinic is provide friendly, caring dentistry and highest level of general, cosmetic, and specialist dental treatments.': 'هدف عيادتنا هو تقديم طب أسنان ودود ومهتم وأعلى مستوى من العلاجات العامة والتجميلية والمتخصصة للأسنان.',
    
    // Contact Page
    'Contact': 'اتصل',
    'contact us': 'اتصل بنا',
    'Get In Touch': 'تواصل معنا',
    'Contact Information': 'معلومات الاتصال',
    'Send Message': 'إرسال الرسالة',
    'Your Name': 'اسمك',
    'Your Email': 'بريدك الإلكتروني',
    'Your Phone': 'هاتفك',
    'Subject': 'الموضوع',
    'Your Message': 'رسالتك',
    
    // About Page
    'About Our Clinic': 'عن عيادتنا',
    'Our Mission': 'مهمتنا',
    'Our Vision': 'رؤيتنا',
    'Our Values': 'قيمنا',
    
    // Services Page
    'All Services': 'جميع الخدمات',
    'Service Details': 'تفاصيل الخدمة',
    'Book Appointment': 'حجز موعد',
    
    // Team Page
    'Meet Our Team': 'تعرف على فريقنا',
    'Professional Staff': 'طاقم مهني',
    'Our Team': 'فريق العمل',
    'team': 'الفريق',
    
    // Gallery Page
    'Our Gallery': 'معرضنا',
    'Photo Gallery': 'معرض الصور',
    'Gallery': 'المعرض',
    'gallery': 'المعرض',
    'Ceramic Clinic - Gallery': 'عيادة السيراميك - المعرض',
    
    // FAQ Page
    'Frequently': 'الأسئلة',
    'Asked': 'المتكررة',
    'Questions': 'الشائعة',
    'Frequently Asked Questions': 'الأسئلة الشائعة',
    'Common Questions': 'أسئلة شائعة',
    'faqs': 'الأسئلة الشائعة',
    
    // Contact Page
    'Contact Us': 'اتصل بنا',
    'contact us': 'اتصل بنا',
    'contact info': 'معلومات الاتصال',
    'Connecting': 'تواصل',
    'Near & Far': 'قريب وبعيد',
    'For emergency dental care or to schedule an appointment, contact our office at visit our clinic.': 'للرعاية الطارئة للأسنان أو لحجز موعد، اتصل بمكتبنا أو زر عيادتنا.',
    'visit us on': 'زرنا في',
    'working hours': 'ساعات العمل',
    'email us': 'راسلنا عبر البريد الإلكتروني',
    
    // Blog Page
    'Latest News': 'أحدث الأخبار',
    'Blog Posts': 'مقالات المدونة',
    'Read More': 'اقرأ المزيد',
    
    // Page Titles
    'About': 'من',
    'Us': 'نحن',
    'About Us': 'من نحن',
    'Services': 'الخدمات',
    'Team': 'الفريق',
    'Gallery': 'المعرض',
    'FAQs': 'الأسئلة الشائعة',
    'Contact': 'اتصل بنا',
    'Blog': 'المدونة',
    'Service Details': 'تفاصيل الخدمة',
    '404 Not Found': '404 الصفحة غير موجودة',
    
    // Common
    'Loading...': 'جاري التحميل...',
    'Submit': 'إرسال',
    'Cancel': 'إلغاء',
    'Save': 'حفظ',
    'Edit': 'تعديل',
    'Delete': 'حذف',
    'Search': 'بحث',
    'Filter': 'تصفية',
    'Sort': 'ترتيب',
    'Previous': 'السابق',
    'Next': 'التالي',
    'Back': 'رجوع',
    'Close': 'إغلاق',
    'Open': 'فتح',
    'Yes': 'نعم',
    'No': 'لا',
    'Success': 'نجح',
    'Error': 'خطأ',
    'Warning': 'تحذير',
    'Info': 'معلومات',
    
    // Time
    'Today': 'اليوم',
    'Yesterday': 'أمس',
    'Tomorrow': 'غداً',
    'This Week': 'هذا الأسبوع',
    'Last Week': 'الأسبوع الماضي',
    'This Month': 'هذا الشهر',
    'Last Month': 'الشهر الماضي',
    'This Year': 'هذا العام',
    'Last Year': 'العام الماضي',
    
    // Months
    'January': 'يناير',
    'February': 'فبراير',
    'March': 'مارس',
    'April': 'أبريل',
    'May': 'مايو',
    'June': 'يونيو',
    'July': 'يوليو',
    'August': 'أغسطس',
    'September': 'سبتمبر',
    'October': 'أكتوبر',
    'November': 'نوفمبر',
    'December': 'ديسمبر',
    
    // Days
    'Sunday': 'الأحد',
    'Monday': 'الاثنين',
    'Tuesday': 'الثلاثاء',
    'Wednesday': 'الأربعاء',
    'Thursday': 'الخميس',
    'Friday': 'الجمعة',
    'Saturday': 'السبت'
};
